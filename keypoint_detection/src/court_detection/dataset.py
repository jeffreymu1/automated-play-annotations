...
class DeepSportDataset(Dataset):
    ...
    def _load_player_annotations(self, json_path: Path, calib: CameraCalibration) -> PlayerAnnotations:
        ...
        foot_positions = self._load_player_foot_positions(json_path, calib)
        return PlayerAnnotations(
            boxes=torch.from_numpy(boxes),
            foot_positions=torch.from_numpy(foot_positions),
        )

    @staticmethod
    def _load_player_boxes(json_path: Path) -> np.ndarray:
        ...
        if mask is None:
            raise FileNotFoundError(f"Could not read player mask: {mask_path}")

        boxes: list[tuple[float, float, float, float]] = []
        for value in np.unique(mask):
            instance_id = int(value)
            if instance_id == 0 or instance_id // 1000 != 1:
                continue
    ...
    @staticmethod
    def _load_player_foot_positions(json_path: Path, calib: CameraCalibration) -> np.ndarray:
        with open(json_path) as f:
            data = json.load(f)

        points_w: list[np.ndarray] = []
        for player in data.get("players", []):
            if player.get("status", 1) != 1:
                continue
            pos_feet = player.get("pos_feet")
            if pos_feet is None:
                continue
            points_w.append(np.array([pos_feet[0], pos_feet[1], 0.0], dtype=float))

        view = data.get("view")
        for ann in data.get("annotations", []):
            if ann.get("type") != "Player":
                continue
            if view is not None and ann.get("camera") not in (None, view):
                continue
            foot1 = ann.get("foot1")
            foot2 = ann.get("foot2")
            if foot1 is None or foot2 is None:
                continue
            points_w.append((np.asarray(foot1, dtype=float) + np.asarray(foot2, dtype=float)) * 0.5)

        if not points_w:
            return np.zeros((0, 2), dtype=np.float32)

        uvs = project_world_to_image(np.stack(points_w, axis=0), calib)
        in_image = (
        ...
        return uvs[in_image].astype(np.float32).reshape(-1, 2)
