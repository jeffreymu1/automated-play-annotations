...
class Stage1Config:
    ...
    line_confidence_threshold: float = 0.3
    class_confidence_threshold: float = 0.3
...
def fit_lines_from_heatmaps(
    ...
    for class_id, name in enumerate(line_names[: class_probs.shape[0]]):
        ...
        mask = _confidence_mask(lineness, class_probs, class_id, prior, config)
...
def _confidence_mask(
    ...
    raise ValueError(f"Unknown Stage 1 confidence mode: {mode}")
