...
def _render_line_targets(
    ...
    if foul_line_occlusion_names:
        for occluder_name in set(foul_line_occlusion_names.values()):
            uv = lines_uv.get(occluder_name)
            ...
            if finite.sum() < 3:
                continue
            pts = np.round(uv_out[finite]).astype(np.int32)
            mask = np.zeros((out_h, out_w), dtype=np.uint8)
            cv2.fillPoly(mask, [pts.reshape(-1, 1, 2)], color=1, lineType=cv2.LINE_8)
            if target_mask is not None:
                mask[~target_mask] = 0
            occlusion_masks[occluder_name] = mask.astype(bool)

    class_lookup = {name: i for i, name in enumerate(line_names)}
    for geometry_name, uv in lines_uv.items():
        name = class_by_geometry.get(geometry_name, geometry_name)
        class_idx = class_lookup.get(name)
        ...
        if foul_line_occlusion_names and geometry_name in foul_line_occlusion_names:
            occlusion = occlusion_masks.get(foul_line_occlusion_names[geometry_name])
...
def _render_side_target(
    ...
    out_h = max(1, image_h // output_stride)
    out_w = max(1, image_w // output_stride)
    halfcourt = lines_uv.get("halfcourt")
    court_full = lines_uv.get("__court_full")
    if halfcourt is None:
        empty = np.zeros((out_h, out_w), dtype=np.float32)
    ...
    right_ref = lines_uv.get("baseline_right")
    right_sign = 1.0
    if right_ref is not None:
        ref = right_ref / output_stride
        ref_finite = np.isfinite(ref).all(axis=1)
        if ref_finite.any():
            r = ref[ref_finite].mean(axis=0)
            value = direction[0] * (r[1] - p0[1]) - direction[1] * (r[0] - p0[0])
            right_sign = 1.0 if value >= 0.0 else -1.0

    yy, xx = np.indices((out_h, out_w), dtype=np.float32)
    signed = (direction[0] * (yy - p0[1]) - direction[1] * (xx - p0[0])) / norm
    signed *= right_sign
    if blur_sigma > 0.0:
        scaled = np.clip(signed / blur_sigma, -60.0, 60.0)
        side_target = (1.0 / (1.0 + np.exp(-scaled))).astype(np.float32)
    else:
        side_target = (signed >= 0.0).astype(np.float32)

    side_weight = np.zeros((out_h, out_w), dtype=np.float32)
    if court_full is not None:
        court_out = court_full / output_stride
        court_finite = np.isfinite(court_out).all(axis=1)
        if court_finite.sum() >= 3:
            court_pts = np.round(court_out[court_finite]).astype(np.int32)
            cv2.fillPoly(side_weight, [court_pts.reshape(-1, 1, 2)], color=1.0, lineType=cv2.LINE_8)
    if target_mask is not None:
        side_weight[~target_mask] = 0.0
    side_target[side_weight <= 0.0] = 0.0
    return side_target, side_weight
...
class CourtLineFrameDataset(Dataset):
    ...
    def _project_lines(self, calib: CameraCalibration) -> dict[str, np.ndarray]:
        worlds = court_markings_world(n=self.n_samples_per_line)
        out: dict[str, np.ndarray] = {}
        needed = {"halfcourt", "baseline_right"}
        for geometry_name, class_name in self.class_by_geometry.items():
            if class_name in self.line_names:
                needed.add(geometry_name)
        for geometry_name in sorted(needed):
            samples = worlds[geometry_name]
            uv = project_world_to_image(samples, calib)
            out[geometry_name] = uv.astype(np.float32)
        out["__court_full"] = project_world_to_image(
            np.array([
                [0.0, 0.0, 0.0],
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        side_target, side_weight = _render_side_target(
            lines_uv,
            image_size=self.image_size,
            output_stride=self.output_stride,
            blur_sigma=self.side_blur_sigma,
            visible_mask=target_mask,
        )
...
class FrozenDinoUNet(nn.Module):
    """Frozen DINOv3 ConvNeXt backbone with separate marking and court-layout decoders."""
    ...
    def __init__(
        ...
        self.layout_proj4 = nn.Conv2d(ch[2], layout_d, kernel_size=1)
        self.layout_proj5 = nn.Conv2d(ch[3], layout_d, kernel_size=1)
        self.layout_dec_c4 = _conv_block(2 * layout_d, layout_d)
        self.layout_refine = _conv_block(layout_d, layout_d)

        self.line_head = nn.Conv2d(d, 1, kernel_size=1)
        self.class_head = nn.Conv2d(d, num_classes, kernel_size=1)
        self.side_head = nn.Conv2d(layout_d, 1, kernel_size=1)
    ...
    def forward(self, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        layout4 = self.layout_proj4(c4)
        layout5 = self.layout_proj5(c5)
        layout5_up = F.interpolate(layout5, size=layout4.shape[-2:], mode="bilinear", align_corners=False)
        layout = self.layout_dec_c4(torch.cat([layout4, layout5_up], dim=1))
        layout = self.layout_refine(layout)

        line_logits = self.line_head(s2).squeeze(1)
        class_logits = self.class_head(s2)
        side_logits = self.side_head(layout).squeeze(1)
        court_logits = self.court_head(layout).squeeze(1)
        return line_logits, class_logits, side_logits, court_logits
...
class CourtLineLightning(L.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.line_names = tuple(line_names) if line_names is not None else MARKING_CLASS_NAMES[:num_classes]
    ...
    def predict(self, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        line_logits, class_logits, side_logits, court_logits = self(images)
        return (
            torch.sigmoid(line_logits),
            torch.softmax(class_logits, dim=1),
            torch.sigmoid(side_logits),
            torch.sigmoid(court_logits),
        )
    ...
    def _shared_step(self, batch: dict[str, Any], stage: str) -> torch.Tensor:
        ...
        if line_logits.shape[-2:] != target_line.shape[-2:]:
            line_logits = F.interpolate(
                line_logits.unsqueeze(1), size=target_size, mode="bilinear", align_corners=False
            ).squeeze(1)
        if class_logits.shape[-2:] != target_line.shape[-2:]:
            class_logits = F.interpolate(
                class_logits, size=target_size, mode="bilinear", align_corners=False
            )
        if side_logits.shape[-2:] != target_line.shape[-2:]:
            side_logits = F.interpolate(
                side_logits.unsqueeze(1), size=target_size, mode="bilinear", align_corners=False
            ).squeeze(1)
        if court_logits.shape[-2:] != target_line.shape[-2:]:
            court_logits = F.interpolate(
                court_logits.unsqueeze(1), size=target_size, mode="bilinear", align_corners=False
            ).squeeze(1)
        ...
        side_raw = F.binary_cross_entropy_with_logits(side_logits, target_side, reduction="none")
        if target_side_weight is None:
            target_side_weight = torch.ones_like(target_side)
        side_loss = (side_raw * target_side_weight).sum() / target_side_weight.sum().clamp_min(1.0)
        loss = (
            line_loss
            + self.hparams.lambda_class * class_loss
            + self.hparams.lambda_side * side_loss
