"""Debug structured heatmap homography RANSAC on images or image folders."""
...
def _make_config(args: argparse.Namespace) -> StructuredRansacConfig:
    return StructuredRansacConfig(
...
def _result_diagnostics(result: StructuredHomographyResult) -> dict:
    return {
...
def _save_heatmap_overlay(
    ...
    overlay = overlay_line_predictions(image, class_probs, lineness, palette)
...
def _process_image(
    ...
    refine_image = full_image
    line_prob, class_probs = _predict_on_image(model, refine_image, device, image_size)
    config = _make_config(args)
    result = fit_homography_from_heatmaps(refine_image, line_prob, class_probs, config)

    stem = args.name if args.name and args.image is not None else image_path.stem
    heatmap_path = args.out / f"{stem}_heatmap.png"
    line_debug_path = args.out / f"{stem}_structured_lines.png"
    template_path = args.out / f"{stem}_structured_template.png"
    json_path = args.out / f"{stem}_structured.json"
    _save_heatmap_overlay(heatmap_path, refine_image, line_prob, class_probs, palette)
    _save_line_debug_overlay(line_debug_path, refine_image, line_prob, class_probs, result, config, palette)
    _save_template_overlay(template_path, refine_image, result, palette)
    ...
    print(
        f"{image_path.name}: structured={result.success} score={result.score:.4f}; "
        f"wrote {template_path.name}"
    )
    return frame_diag
...
def run(args: argparse.Namespace) -> None:
    ...
    folder_json = args.out / "structured_diagnostics.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out", type=Path, default=Path("results/structured_homography_debug"))
    ...
    parser.add_argument("--structured-ransac-iter", type=int, default=500)
    ...
    parser.add_argument("--sample-line-count", type=int, default=5)
