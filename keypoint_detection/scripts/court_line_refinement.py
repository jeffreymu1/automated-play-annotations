    ...
    return {
        "stage1_lines": {
            name: {
                ...
                "fit_residual": fit.fit_residual,
                "score": fit.score,
                "num_inliers": int(len(fit.inliers)),
            }
            for name, fit in line_fits.items()
        },
        "candidate_pixels": {
            name: int(_stage1_confidence_mask(lineness, class_probs, class_id, config).sum())
...
def _stage1_confidence_mask(
    ...
    if config.confidence_mode == "joint":
        return prior > config.pixel_threshold
    if config.confidence_mode == "separate":
        return (
            lineness > config.line_confidence_threshold
        ) & (
            class_probs[class_id] > config.class_confidence_threshold
        )
    if config.confidence_mode == "argmax":
        return (
            prior > config.pixel_threshold
        ) & (
            class_probs.argmax(axis=0) == class_id
        )
    raise ValueError(f"Unknown Stage 1 confidence mode: {config.confidence_mode}")


def _save_stage1_debug_overlay(
    path: Path,
    image: np.ndarray,
    lineness: np.ndarray,
    class_probs: np.ndarray,
    ...
    for class_id, name in enumerate(LINE_NAMES[: class_probs.shape[0]]):
        mask = _stage1_confidence_mask(lineness, class_probs, class_id, config)
        if mask.any():
            color = np.array(255 * palette[class_id], dtype=np.uint8)
            overlay[mask] = (0.45 * overlay[mask] + 0.55 * color).astype(np.uint8)

    for fit in line_fits.values():
        color = tuple(int(255 * c) for c in palette[fit.class_id])
        ...
        cv2.circle(overlay, tuple(p0), 4, color=color, thickness=-1, lineType=cv2.LINE_AA)
        cv2.circle(overlay, tuple(p1), 4, color=color, thickness=-1, lineType=cv2.LINE_AA)
...
def _make_config(args: argparse.Namespace, scale: float) -> RefinementConfig:
    return RefinementConfig(
        stage1=Stage1Config(
            ...
            line_confidence_threshold=args.line_confidence_threshold,
            class_confidence_threshold=args.class_confidence_threshold,
...
def visualize(args: argparse.Namespace) -> None:
    ...
    if args.stage1_only:
        line_fits = fit_lines_from_heatmaps(refine_image, line_prob, class_probs, config.stage1)
        args.out.mkdir(parents=True, exist_ok=True)
        palette = class_palette(len(LINE_NAMES))
        stem = args.name or f"stage1_{args.index:03d}"
        overlay_path = args.out / f"{stem}_stage1_lines.png"
        json_path = args.out / f"{stem}_stage1.json"
        _save_stage1_debug_overlay(overlay_path, refine_image, line_prob, class_probs, line_fits, config.stage1, palette)
        json_path.write_text(json.dumps(_stage1_diagnostics(line_fits, line_prob, class_probs, config.stage1), indent=2))
        print(f"wrote {overlay_path}")
        print(f"wrote {json_path}")
        print(f"stage1 lines: {', '.join(line_fits) or 'none'}")
        return
    ...
    fig_dpi = 600 if args.hidpi else 150
    fig, axes = plt.subplots(2, 2, figsize=(14, 9), dpi=fig_dpi)
...
def visualize_image_folder(args: argparse.Namespace) -> None:
    ...
    for i, image_path in enumerate(image_paths):
        ...
        if args.stage1_only:
            line_fits = fit_lines_from_heatmaps(refine_image, line_prob, class_probs, config.stage1)
            overlay_path = args.out / f"{image_path.stem}_stage1_lines.png"
            _save_stage1_debug_overlay(overlay_path, refine_image, line_prob, class_probs, line_fits, config.stage1, palette)
            frame_diag = _stage1_diagnostics(line_fits, line_prob, class_probs, config.stage1)
            frame_diag.update(
                {
                    "frame_index": i,
            ...
            print(
                ...
                f"stage1={','.join(line_fits) or 'none'}"
            )
            continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--hidpi", action="store_true", help="Save the stage visualization at 600 DPI for print.")
    parser.add_argument("--cpu", action="store_true")
    parser.add_argument("--stage1-only", action="store_true", help="Only fit and visualize image-space lines; skip homography and distortion.")

    parser.add_argument("--confidence-mode", choices=("joint", "separate", "argmax"), default="joint")
    parser.add_argument("--pixel-threshold", type=float, default=0.3)
    parser.add_argument("--line-confidence-threshold", type=float, default=0.3)
    parser.add_argument("--class-confidence-threshold", type=float, default=0.3)
